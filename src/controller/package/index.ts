...
export const addRandomQuestionsFromCourse = async (req: Request, res: Response) => {
  try {
    const { packageId } = req.params;
    const { courseId, count } = req.body as { courseId: string; count: number | string };
    ...
    const existing = await prisma.packageQuestion.findMany({
      where: { packageId },
    ...
    const randomQuestions = await prisma.$queryRaw<Array<{ id: string }>>`
      SELECT q.id
      FROM "question" q
    ...
    const lastOrder = await prisma.packageQuestion.aggregate({
      where: { packageId },
      _max: { order: true },
    });
